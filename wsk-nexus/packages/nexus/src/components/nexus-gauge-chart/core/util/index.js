...
export function calculateSegmentStops({ tickData, customSegmentStops, min, max }) {
  ...
  if (_head(customSegmentStops) !== min) {
    throw new Error(`First value should be equivalent to min value given. Current min value - ${min}`);
  }

  // last element shuold be equivalent to max
  if (_last(customSegmentStops) !== max) {
    throw new Error(`Last value should be equivalent to max value given. Current min value - ${max}`);
  }
...
export function calculateSegmentLabelCount({ maxSegmentLabelCount, segmentCount }) {
  ...
  return _isNumber(max_segment_label_count) && max_segment_label_count >= 0 && max_segment_label_count <= segments_count
...
export function calculateTicks(scale, { min, max, segments }) {
  ...
  const normalize_ticks = (_last(ticks) !== max || segments < ticks.length) && ticks.length > 1;
