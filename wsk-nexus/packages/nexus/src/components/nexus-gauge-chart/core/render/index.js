...
import { centerTranslation, getRadius, calculateNeedleHeight, formatCurrentValueText, sumArrayTill } from '../util';
import { configureArc, configureTicks, configureTickData, configureScale } from '../config/configure';
...
export const render = ({ container, config }) => {
  ...
  const centerTx = centerTranslation(r, config.paddingHorizontal, config.paddingVertical);
...
export function _renderLabels({ config, svg, centerTx, r }) {
  ...
  if (isCustomLabelsPresent && !isCustomLabelsValid) {
    throw new Error(`Custom Segment Labels should be an array with length of ${tickData.length}`);
  }
  ...
  lg.selectAll('text')
    ...
    .append('text')
    .attr('transform', (d, i) => {
      const ratio = config.customSegmentStops.length === 0 ? scale(d) : sumArrayTill(tickData, i);
...
function _renderCustomSegmentLabels({ config, svg, centerTx, r, ticks, tickData, scale, range }) {
...
function _renderNeedle({ config, svg, r, centerTx }) {
  ...
  return pg.append('path').attr('d', pointerLine).attr('transform', `rotate(${config.minAngle})`);
}
